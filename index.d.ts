...
interface ReadlineOptions {
  /**
   * Milliseconds to wait after a carriage return before treating a following linefeed as part of
   * the same line ending, rather than a second line. Defaults to `100`, and is clamped to a minimum
   * of `100`.
   */
  crlfDelay?: number
...
interface Readline<E extends ReadlineEvents = ReadlineEvents> extends Readable<E> {
  ...
  /**
   * Clear the current input line from the output and reset the line buffer and cursor, returning
   * the cleared line.
...
declare class Readline {
   ...
   * @param opts - Options; `input` and `output` are the streams to read from and render to,
   * `prompt` defaults to `'> '`, and `crlfDelay` defaults to `100`.
   */
  constructor(opts?: ReadlineOptions)
}

/**
 * A readable stream that reads input line by line, rendering an editable prompt line with cursor
 * movement, history, and line-editing keys.
 */
